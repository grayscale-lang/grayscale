...
static char gray_test_nonce_buffer[128];

static const char *gray_test_nonce(void) {
    return gray_test_nonce_buffer;
}
...
static char        gray_test_fail_message[1024];
...
_Noreturn void gray_test_vfail(const char *code, const char *file, int line,
    ...
    (void)code;
    vsnprintf(gray_test_fail_message, sizeof(gray_test_fail_message), format, arguments);
    /* Result lines are tab-delimited and newline-terminated; a message that
     * smuggled in either byte would desync the parser. */
    for (char *cursor = gray_test_fail_message; *cursor; cursor++)
        if (*cursor == '\t' || *cursor == '\n' || *cursor == '\r') *cursor = ' ';
...
void gray_test_begin(void) {
    ...
    const char *nonce = getenv("GRAY_TEST_PROTOCOL_NONCE");
    if (nonce) {
        snprintf(gray_test_nonce_buffer, sizeof(gray_test_nonce_buffer), "%s", nonce);
    } else {
        gray_test_nonce_buffer[0] = '\0';
    }
}

void gray_test_run(const char *name, void (*fn)(void), const char *file, int line) {
    gray_test_fail_message[0] = '\0';
    ...
    if (setjmp(gray_test_env) == 0) {
        ...
        printf("%sGRAYTEST FAIL %s\t%s:%d\t%s\n", gray_test_nonce(), name,
               ...
               gray_test_fail_message[0] ? gray_test_fail_message : "test failed");
